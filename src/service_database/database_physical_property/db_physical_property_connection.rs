...
// 确保引入了正确的 Migrator
use crate::service_database::database_physical_property::migration::Migrator;
...
/// **公共初始化入口**
pub async fn initialize_physical_property_db(
    ...
    if let Some(parent) = path.parent() {
        if !parent.exists() {
            eprintln!(
                "📂 [PhysicalPropertyDB] 父目录不存在，正在创建: {:?}",
                parent
            );
    ...
    if !path.exists() {
        eprintln!(
            "🆕 [PhysicalPropertyDB] 数据库文件不存在，创建新文件: {:?}",
            path
        );
        File::create(path)
...
// get_shutter_db 保持不变...
pub async fn get_physical_property_db(
) -> Result<&'static migration_orm::DatabaseConnection, migration_orm::DbErr> {
    match DB.get() {
        Some(db_conn) => Ok(db_conn),
        None => Err(migration_orm::DbErr::Custom(
            "PhysicalPropertyDB Database not initialized.".to_string(),
        )),
    }
}

pub async fn close_physical_property_db() {
    ...
    if let Some(db) = DB.get() {
        ...
        if let Err(e) = db
            ...
            .await
        {
            eprintln!("⚠️ [PhysicalPropertyDB] WAL checkpoint 失败: {}", e);
        }
    ...
    eprintln!("✅ [PhysicalPropertyDB] WAL 已安全落盘");
}
