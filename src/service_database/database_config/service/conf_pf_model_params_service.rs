...
pub async fn select_conf_pf_model_params_one() -> Result<ConfPfModelParamsDto, DbErr> {
    let db = get_config_db().await.unwrap(); // 获取数据库连接

    let model = ConfPfModelParamsEntity::find().one(db).await?;

    // 将查询结果 (Option<ConfPfModelParamsModel>) 转换为 Option<ConfPfModelParamsDto>
    let dto: ConfPfModelParamsDto = model.map(ConfPfModelParamsDto::from).unwrap();
...
pub async fn upsert_and_insert_fixed_conf_pf_model_params(
    ...
    let db = get_config_db().await.unwrap(); // 获取数据库连接
    let model_one = ConfPfModelParamsEntity::find().one(db).await?;

    if let Some(model) = model_one {
        // 更新逻辑
        let mut active_model: ConfPfModelParamsActiveModel = model.clone().into_active_model();
        active_model.code = Set(data.code);
        active_model.name = Set(data.name);
        active_model.solver_type = Set(data.solver_type);
        active_model.params = Set(data.params);
        ...
        let active_model = ConfPfModelParamsActiveModel {
            ...
            code: Set(data.code),
            name: Set(data.name),
            solver_type: Set(data.solver_type),
