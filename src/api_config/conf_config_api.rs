...
use crate::database_config::service::conf_config_service::*;
...
pub async fn get_conf_config_one_message_api() -> Result<ConfigDto> {
    // 调用服务层函数，并处理 DbErr
    let result = select_fixed_conf_config().await.map_err(handle_db_err)?;
