...
// 确保引入了正确的 Migrator
use crate::database_shutter::migration::Migrator;
...
pub async fn initialize_shutter_db(
    ...
    if !path.exists() {
        ...
        eprintln!("📂 [ShutterDB] 检测到现有数据库文件: {:?}", path);
    }
    ...
    DB.get_or_try_init(|| async {
        ...
        eprintln!("🚀 [ShutterDB] 开始执行 Migration...");
        Migrator::up(&db, None).await.map_err(|e| {
            eprintln!("❌ [ShutterDB] Migration 失败: {}", e);
            e
        })?;
        eprintln!("✅ [ShutterDB] Migration 执行完成");
